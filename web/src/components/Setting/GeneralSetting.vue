...
const {
  isLoading: sgUpdIsLoading,
  error: sgUpdError,
  execute: sgUpdExecute,
} = useFetch("PUT", "/api/setting");
  ...
  <article>
    <form @submit.prevent="sgUpdate">
      ...
      <textarea
        ...
        id="input-about"
        rows="10"
        placeholder="This is my simple site"
        v-model="sgFormData.siteAbout"
      ></textarea>
      ...
      <fieldset>
        <label for="input-hide-setting">
          <input
            type="checkbox"
            id="input-hide-setting"
            name="hide_setting"
            role="switch"
            v-model="sgFormData.hideSetting"
          />
          Hide setting page
        </label>
      </fieldset>
